...
echo ""
echo "----------- 7.3.Overview-of-Device-and-Module-Handling.sh -----------"
echo ""
...
#Udev will only load a module if it has a bus-specific alias and the bus driver properly exports the necessary aliases to sysfs. In other cases, one should arrange module loading by other means. With Linux-5.2.8, Udev is known to load properly-written drivers for INPUT, IDE, PCI, USB, SCSI, SERIO, and FireWire devices.
...
#If there is no modalias file in the relevant directory under /sys/bus, this means that the kernel developers have not yet added modalias support to this bus type. With Linux-5.2.8, this is the case with ISA busses. Expect this issue to be fixed in later kernel versions.
